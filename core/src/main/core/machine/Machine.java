...
public class Machine {
    ...
    /**
     * Override this method to specify whether the machine can be placed on a specific tile.
     * @return true if the machine can be placed on the tile, false otherwise.
     */
    ...
    public final boolean pushLoadTo(Machine target) {
        ...
        float fromX = output.currentX;
        float fromY = output.currentY;
        float speed = getOutputSpeed() * timeEfficient;

        if (!target.acceptLoad(this, output)) return false;

        item = null;
        group.reportTransfer(this, target, output.type, fromX, fromY, speed);
        return true;
    }

    public final Item extractOutputLoad(Item expected) {
        if (expected == null || item != expected || !canPushLoad()) {
            return null;
        }
        item = null;
        return expected;
...
//        load.currentY = centerY;
    ...
    public boolean canAcceptLoad(Machine source, Item load) {
        return source != null && canAcceptLoad(load);
    }

    public boolean acceptLoad(Machine source, Item load) {
        return canAcceptLoad(source, load) && acceptLoad(load);
    }

    public boolean acceptLoad(Item load) {
        return canAcceptLoad(load) && handleItem(load.type);
    }
